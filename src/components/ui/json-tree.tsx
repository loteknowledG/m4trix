...
export default function JsonTree({ data, depth = 0 }: JsonTreeProps) {
  ...
  if (Array.isArray(data)) {
    return <JsonArrayNode data={data} depth={depth} />;
  }

  if (isPlainObject(data)) {
    return <JsonObjectNode data={data} depth={depth} />;
  }
...
function JsonArrayNode({ data, depth }: JsonArrayNodeProps) {
...
function JsonObjectNode({ data, depth }: JsonObjectNodeProps) {
