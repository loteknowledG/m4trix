...
export async function speakWithJennyVoice(text: string) {
  const spokeWithCoderoboJenny = await speakViaCoderobo(text);
  if (spokeWithCoderoboJenny) {
    return true;
  }

  const spokeWithJennyBrowserVoice = await speakWithBrowserVoice(text, JENNY_VOICE_PATTERNS);
  if (spokeWithJennyBrowserVoice) {
    return true;
  }

  return speakWithFallbackVoice(text);
}
