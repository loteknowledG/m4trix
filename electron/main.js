  ...
  for (const it of arr) {
    ...
    if (typeof it === 'object') {
      ...
      if (urlArr && urlArr.length) {
        const first =
          urlArr.find(u => typeof u === 'string' && /^https?:\/\//.test(u)) || urlArr[0];
...
async function getGooglePhotosFetcher() {
  ...
  try {
    ...
    gpFetcher =
      cjs?.default ||
      (typeof cjs === 'function' ? cjs : null) ||
      cjs?.fetchAlbum ||
      cjs?.fetchAlbumImageUrls ||
      cjs?.getAlbumImageUrls ||
      cjs?.getImageUrls ||
      cjs?.getUrls ||
      cjs?.fetchImageUrls;
    ...
    try {
      ...
      gpFetcher =
        esm?.default ||
        (typeof esm === 'function' ? esm : null) ||
        esm?.fetchAlbum ||
        esm?.fetchAlbumImageUrls ||
        esm?.getAlbumImageUrls ||
        esm?.getImageUrls ||
        esm?.getUrls ||
        esm?.fetchImageUrls;
  ...
  if (!gpFetcher) {
    try {
      ...
      gpFetcher =
        ...
        mod?.fetchAlbum ||
        mod?.fetchAlbumImageUrls ||
        mod?.getAlbumImageUrls ||
        mod?.getImageUrls ||
        mod?.getUrls ||
        mod?.fetchImageUrls ||
...
ipcMain.handle('gp-fetch-album', async (_evt, albumUrl) => {
  ...
  try {
    console.log('[electron-main] gp-fetch-album result', {
      total: urls.length,
